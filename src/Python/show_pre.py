...
data_var = ds.metpy.parse_cf('pre')
...
pre = data_filtered.sel(lon=slice(96, 106), lat=slice(4, 21), time=str(year))
data_avg = pre['pre'].mean(dim='time')

x = pre.lon
y = pre.lat

ax = axs
mp = ax.imshow(data_avg, extent=(x.min(), x.max(), y.min(), y.max()), cmap='viridis', origin='lower')
