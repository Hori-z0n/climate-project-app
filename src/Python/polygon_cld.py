...
ds = xr.open_dataset('C:/Users/konla/OneDrive/Desktop/Weather/cru_ts4.08.1901.2023.cld.dat.nc')

year = 1901

cld = ds.sel(lon=slice(96, 106), lat=slice(4, 21), time=str(year))
time_values = cld['time'].values
...
for time_index, time in enumerate(time_dates):  # วนลูปสำหรับแต่ละเดือน
    month = time.month
    cld_in_month = cld.isel(time=time_index)  # เลือกข้อมูลของเดือนนั้น
    cld_values = cld_in_month['cld'].values  # ค่าอุณหภูมิในเดือนนั้น [lat, lon]
    
    for i, lon_value in enumerate(lon):
        for j, lat_value in enumerate(lat):
            cloud = cld_values[j, i]  # ใช้ดัชนี [lat, lon]
            if not pd.isnull(cloud):  # ตรวจสอบว่าไม่มี NaN
                ...
                features.append({
                    ...
                    "properties": {
                        "CLD": float(cloud),  # แปลง float32 เป็น float
...
output_file = "./src/Geo-data/Year-Dataset/data_cld_1901.json"
...
for month in range(1, 13):
    ...
    for region in province_coord(): 
        for province in region:
            ...
            avg_cld, province_shape = calculate_weighted_cloud(name, shapefile, monthly_data)

            
            if avg_cld is not None and province_shape is not None:
            ...
            count += 1
            print(f"{count}: Month {month}, Province: {name}, Avg Temp: {avg_cld:.3f}")

output_geojson_path = "./src/Geo-data/Year-Dataset/province_cld_1901.json"
...
thailand = gpd.read_file("./src/Geo-data/Year-Dataset/province_cld_1901.json")
fig, ax = plt.subplots(figsize=(10, 10))

# thai_source.plot(column='temperature', cmap='jet', linewidth=0.5, ax=ax, edgecolor='black', legend=True)

thailand.plot(column='CLD', ax=ax, legend=True, cmap='jet', legend_kwds={'label': "Temperature (°C)", 'orientation': "horizontal"})
