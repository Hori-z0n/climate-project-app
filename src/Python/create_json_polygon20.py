...
provinces=['Amnat Charoen', 'Ang Thong', 'Bangkok Metropolis']
times = ["1901-01-16", "1901-02-15", "1901-03-16"]
...
# times = ["1901-01-16", "1901-02-15", "1901-03-16", "1901-04-16", "1901-05-16", "1901-06-16", "1901-07-16", "1901-08-16", "1901-09-16", "1901-10-16", "1901-11-16", "1901-12-16", "1902-01-16", "1902-02-15", "1902-03-16", "1902-04-16", "1902-05-16", "1902-06-16", "1902-07-16", "1902-08-16",
...
os.chdir('C:/Users/konla/OneDrive/Desktop/json/')
for province in provinces:        
        thailand = shapefile[shapefile['NAME_1'] == province]
        data = gpd.read_file(f'C:/Users/konla/OneDrive/Desktop/json/cru_data_{province}.json')
        # data.plot(ax=ax, column='pre')
        # thailand.geometry.boundary.plot(ax=ax, color='black', linewidth=0.5)
        # plt.show()
# with open(f'C:/Users/konla/OneDrive/Desktop/json/cru_data_{provinces[0]}.json', 'r') as f:
#     data = json.load(f)
...
# precipitation = []
        ...
        for time in times:
                ...
                count = 1
                li = {}
                for idx, grid in dataset.iterrows():
                        # print(grid)
                        station.append(f"station{count}")
                        coord.append(f"coord{count}")
                        li.update({f"station{count}":grid.pre, f"coord{count}":grid.geometry})
                        count += 1
                grid_value.append(li)

        station = sorted(list(set(station)))
        coord = sorted(list(set(coord)))
        if len(station) == len(coord):
                pass
        else:
                print('error')
# for i in range(len(times)):
#         for j in range(len(station)):
#                 print(times[i], station[j], grid_value[i][station[j]], grid_value[i][coord[j]])
        # for j in station:
        #         print(times[i], grid_value[i][j])
        # for j in coord:
        #         print(times[i], grid_value[i][j])
        os.makedirs(f'{province}')
        os.chdir(f'{province}')
        count = 1
        for j in range(len(station)):
                features = []
                for i in range(len(times)):
                        ...
                        features.append({
                        "type": "Feature",
                                "geometry": mapping(grid_value[i][coord[j]]),
                                "properties": {
                                "name": province,
                                "time": str(times[i])[0:10],
                                "pre": float(grid_value[i][station[j]]),
                                }
                        })
                geojson_data = {
                ...
                # print(data)
                output_file = f"grid_data_{province}_{count}.json"
...
#     thailand = shapefile[shapefile['NAME_1'] == province]
#     thailand.geometry.boundary.plot(ax=ax, color='black', linewidth=0.5)
#     grid = gpd.read_file(f'C:/Users/konla/OneDrive/Desktop/json/cru_data_{province}.json')
#     grid = grid[grid['time'] == '1901-01-16']
#     grid.plot(ax=ax, column='pre', legend=True)
