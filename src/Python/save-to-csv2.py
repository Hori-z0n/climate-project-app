...
times = '1960'
# year = str(times)[0:10]
# for date in tqdm(tmx_data.sel(time = str(times))['time'], ascii=False, ncols=75, leave=False):
for date in tqdm(pre_data.sel(time = str(times))['time'], ascii=False, ncols=75, leave=False):
    ymd = str(date.values)[0:10]
    
    data_filtered = tmx_data.sel(time=ymd)
    
    dates.append(ymd)
    
    data_avg = data_filtered['mx2t'].mean(dim='time')
            ...
            if not pd.isnull(precipitation):  # ตรวจสอบว่าไม่มี NaN
                ...
                features.append({
                    ...
                    "properties": {
                        ...
                        # "precipitation": float(precipitation) - 273.15  # แปลง float32 เป็น float
                    }
                })
                # print(grid_polygon)
