...
data_filtered = ds.sel(time=slice('1901-01-01', '2023-12-31'))

fig, axs = plt.subplots(nrows=1, ncols=1, figsize=(18, 12), subplot_kw={'projection': ccrs.PlateCarree()})

frs = data_filtered.sel(lon=slice(90, 110), lat=slice(0, 50), time=str(year))
...
ax.set_extent([90, 110, 0, 50], crs=ccrs.PlateCarree())
...
shp_int.geometry.boundary.plot(ax=ax, color='white', linewidth=0.5)
