...
shapefile = gpd.read_file('C:/Users/AlfredHugh/Desktop/Climate/Geo-data/thailand-Geo.json')
...
for year in range(start_year, stop_year):
    ...
    print(f"\nGeoJson {count} grid is create saved complete.")

    for month in tqdm(ary_month, desc="Create polygon...", leave=False, ncols=75):
        ...
        if monthly_data.empty == False:
            for day in tqdm(_day, leave=False):
                ...
                for region in tqdm(province_coord(), desc="Loading region & province...", ascii=False, ncols=75, colour='yellow'):
                    for province in region:
                        ...
                        avg_tmn, province_shape1 = calculate_weighted_polygon(name, shapefile, day_data, cru='tmn')
                        avg_tmx, province_shape1 = calculate_weighted_polygon(name, shapefile, day_data, cru='tmx')
