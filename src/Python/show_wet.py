...
data_var = ds.metpy.parse_cf('wet')
...
wet = data_filtered.sel(lon=slice(96, 106), lat=slice(4, 21), time=str(year))
data_avg = wet['wet'].mean(dim='time').dt.days
...
plt.title('Rain Days for Thailand in ' + str(year), fontsize=12)
