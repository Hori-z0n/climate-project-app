...
txx_year_value = []
tnn_year_value = []
for year in tqdm(range(start_year, stop_year), ascii=False, ncols=75):
    ...
    for month in tqdm(range(start_month, stop_month), desc="Start Create Json...", ascii=False, ncols=75, colour='orange'):
        for i in tqdm(range(0, 924), desc="Select Year...", ascii=False, ncols=75, colour='orange'):
            features = {
                "type": "Feature",
                "geometry": {
            ...
            second_features.append(features)
        second_features.append({"indices":{'txx': txx_year_value[month-1],'tnn': tnn_year_value[month-1]}})
    geojson_data['features'].append(second_features)
        
    output_geojson_path = f'./src/Geo-data/Year-Dataset/data_{year}_V3.json'
