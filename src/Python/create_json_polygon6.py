...
for year in tqdm(range(start_year, stop_year), desc="Create GeoJson...", ascii=False, ncols=75):
    data1 = pre.sel(lon=slice(96, 106), lat=slice(4, 21), time=str(year))
    time_values1 = data1['time'].values
    time_dates1 = pd.to_datetime(time_values1)
    data2 = tmn.sel(lon=slice(96, 106), lat=slice(4, 21), time=str(year))
    time_values2 = data2['time'].values
    time_dates2 = pd.to_datetime(time_values2)
    data3 = tmx.sel(lon=slice(96, 106), lat=slice(4, 21), time=str(year))
    time_values3 = data3['time'].values
    time_dates3 = pd.to_datetime(time_values3)
    data4 = tmx.sel(lon=slice(96, 106), lat=slice(4, 21), time=str(year))
    time_values4 = data4['time'].values
    time_dates4 = pd.to_datetime(time_values4)

    if len(time_dates1) == len(time_dates2) == len(time_dates3) == len(time_dates4):
    ...
    for time_index, time in tqdm(enumerate(time_dates1), desc="Create time Series...", ascii='0123456789#', ncols=75):
        ...
        for i, lon_value in tqdm(enumerate(lon), desc="Create grid...",unit=' grid', ascii='0123456789#', ncols=75):
            for j, lat_value in tqdm(enumerate(lat), leave=False, ncols=75):
                ...
                if not pd.isnull(diural_temperature_range) and not pd.isnull(precipitation) and not pd.isnull(minimum_temperature) and not pd.isnull(maximum_temperature):  
    ...
    for month in tqdm(range(1, 13), desc="Create month...", ascii=False, ncols=75, colour='green'):
        ...
        # print(monthly_data['day'].values[0])
        # print(monthly_data['month'].values[0])
        # print(monthly_data['year'].values[0])

        _day = str(monthly_data['day'].values[0])
        _month = str(monthly_data['month'].values[0])
        _year = str(monthly_data['year'].values[0])
        ...
        for region in tqdm(province_coord(), desc="Loading region & province...", ascii=False, ncols=75, colour='yellow'): 
            for province in region:
                ...
                if (avg_dtr is not None and province_shape1 is not None) and (avg_pre is not None and province_shape2 is not None) and (avg_tmn is not None and province_shape3 is not None) and (avg_tmx is not None and province_shape5 is not None):
                    feature = {
                        "type": "Feature",
    ...
    output_geojson_path = f'./src/Geo-data/Year-Dataset/data_{year}_V5.json'
