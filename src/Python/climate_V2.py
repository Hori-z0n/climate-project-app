...
class Climate:
    ...
    def NetCDF_SHP_lat_lon(self, name_of_nc, name_of_variable, name_of_lat_var, name_of_lon_var, name_of_shp, box_values, correct_360):
        """
        ...
        self.lat_lon_SHP(name_of_shp, lat, lon, box_values, correct_360)

        # return mapped lat lon (2D lat, lon)
        return lat, lon


    def lat_lon_SHP(self, name_of_shp, lat, lon, box_values, correct_360):
        """
    ...
    def spatial_overlays(self, df1, df2, how='intersection', reproject=True):
        ...
        if how=='intersection':
            ...
            dfinter = self.spatial_overlays(df1, df2, how='intersection')
            dfsym = self.spatial_overlays(df1, df2, how='symmetric_difference')
            dfunion = dfinter.append(dfsym)
            dfunion.reset_index(inplace=True, drop=True)
            return dfunion
        elif how=='identity':
            dfunion = self.spatial_overlays(df1, df2, how='union')
    ...
    def lat_lon_to_index(self, lat_source,
                        lon_source,
                        lat_mapped,
                        lon_mapped):
        """
        ...
        for nc_name in nc_names:
            ...
            with nc4.Dataset(target_name, "w", format="NETCDF4") as ncid: # creating the NetCDF file
                ...
                for i in np.arange(len(varibale_name_list)):
                    ...
                    var_value  = self.weighted_average(nc_name,
                                                target_date_times,
                                                name_of_var_time,
    ...
    def calculate_weighted_polygon(self, province_name, shapefile, shapename, data, idx):

        province_coord = shapefile[shapefile[shapename] == province_name]
    ...
    def calculate_gsl(self):
        pass

    def calculate_percentile_p(self):
        pass

    def calculate_preci_days(self):
        pass

    def calculate_rxnday(self):
        pass

    def calcualte_cdd(self):
        pass

    def calculate_cwd(self):
        pass

    def calculate_sdii(self):
        pass

    # def calculate_spi(self,ds, thresh, dimension, precip_var='tp', threshold_mm=10.0, resample_freq='M'):
    def calculate_spi(self,thresh, dimension, precip_var='tp', threshold_mm=10.0, resample_freq='M'):
        """
        Standardised Precipitation Index or Standardized Precipitation Index is Mwasure of drought using the Standardised Precipitation Index 
        
        on time scales of 3, 6 and 12 months. A drought measure specified as a precipitation deficit.
        
        The input netCDF file should contain daily precipitation data with a variable that can be accessed
        I'm using the `precip_var` parameter to contain.
        
        :param resample_freq: The frequency for resampling the data. Example: 'MS'. Default is None.
        :type resample_freq: str, optional
        
        :param ds: Dataset input to function must be value of netcdf or other etc...
        ...
        try:
            ...
            ds_ma = self.datafile.rolling(time = thresh, center=False).mean(dim=dimension)
            ...
            warnings.warn(f"An error occurred while processing the data: {str(e)}")
            return None
...
# I'm programmer I want to write code for calculate climate extremes indices for my project in python language i using netcdf geojson and csv be a data can you write code about climate extremes indices and show formula equation you use and tell me where resouce that get from be link url and tell me the more much many detail