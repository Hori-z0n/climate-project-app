...
def calculate_weighted_polygon(province_name, shapefile, data, cru):
    ...
    for idx, grid in grid_in_province.iterrows():
        ...
        if (cru != ''):
            ...
            print("You Not select wrong try again")
            break
...
for year in range(start_year, stop_year):
    ...
    # for date in pre_data.sel(time=str(year)).resample(time='M').mean()['time']:
    # for date in pre_data.sel(time=str(year))['time']:

    data_filtered1 = pre_data.sel(time=str(year))
    data_filtered2 = tmn_data.sel(time=str(year))
    data_filtered3 = tmx_data.sel(time=str(year))

    data_avg1 = data_filtered1['tp'].resample(time='M').mean(dim='time')
    data_avg2 = data_filtered2['mn2t'].resample(time='M').mean(dim='time')
    data_avg3 = data_filtered3['mx2t'].resample(time='M').mean(dim='time')

    ymd = (data_avg1['time'].values)[0:10]
    print(ymd)
    # print(data_avg1['time'])
    # print(data_avg1['time'])
    # print(data_avg1['time'])
    # ymd = 
